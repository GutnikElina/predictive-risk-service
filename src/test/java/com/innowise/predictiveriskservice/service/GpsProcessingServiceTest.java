...
class GpsProcessingServiceTest {
    ...
    private RiskScoreCalculationData sampleData() {
        return new RiskScoreCalculationData(UUID.randomUUID(), 1.0, 1.0, 1.0, 1.0);
    }

    @Test
    void processGpsPosition_firstInWindow_highRisk_savesEscalationEventToOutbox() {
        RiskScoreCalculationData data = sampleData();
        ...
        verify(outboxService).save(eq(KafkaEventTypes.RISK_LEVEL_ESCALATED_EVENT), eq("{\"riskScore\":0.9}"));
    }
