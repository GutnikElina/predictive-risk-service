...
class OutboxSchedulerTest {
    ...
    void publishPendingEvents_success_marksEventPublished() {
        ...
        when(kafkaPublisher.sendEvent(eq(event.getKafkaEventType().kafkaTopic), eq(event.getPayload())))
    ...
    void publishPendingEvents_publishFailure_recordsErrorMessage_andDoesNotQuitYet() {
        ...
        verify(outboxService, never()).setStatus(eq(event.getId()), eq(OutboxStatus.QUIT_PUBLISHING));
    }
